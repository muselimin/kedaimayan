...
import {StatusBar, Image, StyleSheet, Dimensions, ScrollView, ImageBackground, TextInput, View, TouchableOpacity, Alert, 
    borderTopLeftRadius, borderTopRightRadius, borderBottomRightRadius, borderBottomLeftRadius} from 'react-native';
...
class ListMenu extends Component{
    constructor(props){
        ...
        this.state={
            ...
            photo: [
                {
                    id: 1,
                    Image : require('../images/beefburguignon.jpeg')
                },
                {
                    id: 2,
                    Image : require('../images/bouillabaisse.jpg')
                },
                {
                    id: 3,
                    Image : require('../images/cassoulet.jpg')
                },
                {
                    id: 4,
                    Image : require('../images/croissant.jpg')
                },
                {
                    id: 5,
                    Image : require('../images/escargot.jpg')
                },
                {
                    id: 6,
                    Image : require('../images/foiegras.jpg')
                },
                {
                    id: 7,
    ...
    toggleModal = () => {
        ...
        if(this.state.statusOrder==0 && this.state.isModalVisible==false){
            this.setState({statusOrder: 1})
            console.log(this.state.statusOrder);
            console.log(this.state.isModalVisible);
        }
        if(this.state.statusOrder==1 && this.state.isModalVisible==true){
            this.setState({statusOrder: 0})
            console.log(this.state.statusOrder);
            console.log(this.state.isModalVisible);
        }
    ...
    componentDidMount(){
        ...
        axios.get('https://belajar-backend-master.herokuapp.com/api/v1/menus')
        .then(res => {
          const categories = res.data;
          this.props.dispatch(getCategories(categories))
        //   console.log(categories)
        })
        axios.get('https://belajar-backend-master.herokuapp.com/api/v1/categories')
        .then(res => {
      ...
      confirm(){
        Alert.alert(
            ...
            axios.post('https://belajar-backend-master.herokuapp.com/api/v1/order',{
                'qty': item.qty,
                'price': item.price,
                'status': item.status,
                'menu_Id': item.id,
            ...
            .then(res => {
                ...
                setTimeout(() => {
                    ...
                    axios.patch(`https://belajar-backend-master.herokuapp.com/api/v1/order/${idOrder}`,{
                        "status": 1
                    })
                    this.state.status=1;
                 },10000);
    ...
    callBill=()=>{
        ...
        this.source.cancel();
        this.setState({statusOrder: 0})
        // axios.patch(`http://192.168.0.19:3000/api/v1//transaction/${this.state.idtransaksi}`,{
        axios.patch(`https://belajar-backend-master.herokuapp.com/api/v1//transaction/${this.state.idtransaksi}`,{
      ...
      getOrders=()=>{
          ...
          this.state.statusOrder===1 ? 
          null :
            
         setInterval(() => {
            
                axios.get(`https://belajar-backend-master.herokuapp.com/api/v1/orderByTransaction/${this.state.idtransaksi}`,{
    ...
    render(){
        ...
        this.state.discount = this.state.subTotal*0.0
        this.state.serviceCharge = this.state.subTotal*0.05
        this.state.tax = this.state.subTotal*0.01
        ...
        return(
            ...
            <View style={styles.container}>
              ...
              {this.props.categories.isLoading === false ? null : <Text>Memuat...</Text>}
              <View style={{backgroundColor:'#2ecc71'}}>
              <View style={{flexDirection: 'row'}}>
              
              <Text style={{fontWeight:'bold', color: 'white'}}>Nomor Meja {tableNumber}</Text>
              <Text style={{fontWeight:'bold', marginLeft: 210, color:'white'}}>Durasi: {this.state.timer}</Text>
              </View>
              <View style={styles.navigation}>
              
                  <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  <TouchableOpacity onPress={()=> this.getCategories(0)}>
                            <Text style={styles.navitem}>Semua</Text>
                        </TouchableOpacity>
                      {this.props.menusdb.data.map(menusdb =>{
                          ...
                          return(
                            ...
                            <TouchableOpacity key = {menusdb.id} onPress={()=> this.getCategories(menusdb.id)}>
                  ...
                  <View style={{marginTop: 23, height: 350, width: '100%'}}>
                      <ScrollView>
            ...
            <View style={{backgroundColor: '#2ecc71', marginTop: 20}}>
                    ...
                    {this.props.menus.map((item)=>{
                        ...
                        return(
                            ...
                            style={{marginLeft: 2, marginTop: 7}} >
                                ...
                                <View style={{backgroundColor: '#2ecc71', margin: 5, borderRadius: 10}}>
...
<View style={{ flex: 1}}>
        ...
        <Modal isVisible={this.state.isModalVisible}>
          ...
          <Button onPress={()=> {this.toggleModal(), this.setState({viewBill: false})}} style={{backgroundColor: '##2ecc71'}}><Text style={{textAlign:'right'}}>X</Text></Button>
            <Text style={{textAlign: 'center', fontWeight:'bold'}}>9 September 2019, 20:09</Text>
            ...
            <TouchableOpacity disabled={this.state.status===0? true: false} style={styles.buttonmodal} onPress={()=> {this.callBill(),this.toggleModal(), this.props.dispatch(clearOrders()) ,this.props.navigation.navigate('Invoice',{tableNumber: this.state.tableNumber})}}><Text style={{color:'white', textAlign: 'center', fontWeight:'bold'}}>BAYAR</Text></TouchableOpacity>
          </View>
                ...
                <View style={{flexDirection: 'row', justifyContent:'space-between', marginBottom: 20}}>
                    <Col><TouchableOpacity disabled={(this.props.menus.length==0? true : false)} style={styles.buttoncontainer} onPress={()=> {this.confirm()}}><Text style={styles.buttontext}>KONFIRM</Text></TouchableOpacity></Col>
                    <Col><TouchableOpacity onPress={()=> this.props.dispatch(clearBill(), this.props.dispatch(clearOrders()))}  disabled={(this.props.menus.length==0? true : false)}style={styles.buttoncontainer}><Text style={styles.buttontext}>PANGGIL</Text></TouchableOpacity></Col>
                    <Col><TouchableOpacity disabled={(this.state.confirm===0? true: false)} style={styles.buttoncontainer} onPress={()=>{this.toggleModal(),this.getOrders()}}><Text style={styles.buttontext}>TAGIHAN</Text></TouchableOpacity></Col>
                </View>
                </View>
              </View>
